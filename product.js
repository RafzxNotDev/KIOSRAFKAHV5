...
export async function initProduct() {
    ...
    if (modal) {
        ...
        // Hentikan kamera jika user close modal saat kamera masih nyala
...
function bindScannerEvents() {
    ...
    if (scanBtn) {
        scanBtn.addEventListener("click", startProductScanner);
    }

    const stopBtn = document.getElementById("stopProductBtn");
    if (stopBtn) {
        stopBtn.addEventListener("click", stopProductScanner);
    }
}
...
async function saveProduct() {
    ...
    const stok = Number(document.getElementById("productStock").value);

    if (isEmpty(nama)) {
        showMessage("Nama produk wajib diisi");
        return;
    }
    if (harga <= 0) {
...
async function loadProducts() {
    ...
    try {
        ...
        snapshot.forEach((item) => {
            renderProduct(item.id, item.data());
        });
        bindTableEvents();
    } catch (err) {
        console.error(err);
