    ...
    try {
        await productScanner.start({ facingMode: "environment" }, { fps: 15, qrbox: { width: 250, height: 250 } },
            async (decodedText) => {
                ...
                fetchProductName(decodedText); // Auto cari nama
            }
        );
    } catch (err) {
...
async function fetchProductName(barcode) {
    ...
    try {
        ...
        if (data.status === 1 && data.product && data.product.product_name) {
            ...
            if (data.product.brands) { nameInput.value = data.product.brands + " - " + data.product.product_name; }
            showMessage("✅ Produk otomatis ditemukan!");
        } else {
        ...
        nameInput.placeholder = originalPlaceholder; showMessage("Gagal terhubung ke server pencarian.");
    }
}
