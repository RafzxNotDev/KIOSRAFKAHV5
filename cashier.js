...
// =====================================
// LOAD PRODUK (DESAIN GRID RAMPING)
// =====================================
async function loadProducts() {
    ...
    snapshot.forEach(item => {
        ...
        // Gunakan grid 4 kolom di layar besar, 2 kolom di layar HP
            ...
            <div class="card product-card h-100 shadow-sm border-0">
                <div class="card-body p-2 d-flex flex-column justify-content-between">
                    <div>
                        ...
                        <div class="text-muted mb-1" style="font-size: 11px;">Stok: ${p.stok}</div>
                    </div>
...
async function addToCart(id) {
    ...
    if (index >= 0) {
        ...
        cart.push({ id, nama: data.nama, harga: data.harga, stok: data.stok, qty: 1 });
    }
    renderCart();
}
...
function increaseQty(index) {
    if (cart[index].qty >= cart[index].stok) { showMessage("Stok tidak mencukupi."); return; }
    ...
    try {
        for (const item of cart) {
            ...
            if (snap.data().stok < item.qty) {
                showMessage("Stok " + snap.data().nama + " tidak mencukupi.");
                await loadProducts(); payBtn.disabled = false; payBtn.innerHTML = '<i class="fa-solid fa-money-bill-wave"></i> Proses Bayar';
                return;
            }
        }
        ...
        for (const item of cart) {
            const ref = doc(db, "products", item.id);
            const snap = await getDoc(ref);
            await updateDoc(ref, { stok: snap.data().stok - item.qty, terjual: (snap.data().terjual || 0) + item.qty });
        }

        await refreshProduct(); await refreshDashboard(); await loadProducts();
